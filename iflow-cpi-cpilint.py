...
def get_package_names(api_url_get_packs, bearer_token=-1, s_user=-1, password=-1):
...
def get_package_names_neo(api_url_get_packs, s_user=-1, password=-1):
...
def get_iflows_names_neo(package_id, api_url_get_iflow, s_user=-1, password=-1):
...
def download_zip_file_neo(api_url_download, id_value, version_value, s_user=-1, password=-1):
...
if __name__ == "__main__":
    ...
    if choose == "2":
        ...
        package_names = get_package_names_neo(api_url_get_packs, s_user, password)
        with ThreadPoolExecutor(max_workers=5) as executor:
            for package_name in package_names:
                iflow_names = get_iflows_names_neo(package_name, api_url_get_iflow, s_user, password)
                iflow_names_new.extend(iflow_names)
                for iflow_name in iflow_names:
                        executor.submit(download_zip_file_neo, api_url_download, iflow_name, version_value, s_user, password)
        if choise == "y":
            run_cpi_lint(iflow_names_new, rules_xml_path)            
